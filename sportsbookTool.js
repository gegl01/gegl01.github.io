(function () {
    ...
    function initSbToolsSelection(scope) {
        ...
        const btClearBetOfTheDays = getElementById("btClearBetOfTheDays");
        const btAddAllToBetOfTheDay = getElementById("btAddAllToBetOfTheDay");
        const notAllBetOfTheDaySelectionsPrematchMessage = getElementById("notAllBetOfTheDaySelectionsPrematchMessage");
        const chkLongBetOfTheDayTitles = getElementById("chkLongBetOfTheDayTitles");

        let betOfTheDayCounter = 0;
        ...
        function listenerForSelection() {
            ...
            if (betslip === previousBetslip) {
                ...
                hide(notAllBetOfTheDaySelectionsPrematchMessage);
            ...
            if (selectionId === previousSelectionId) {
                if (selectionId !== null) {
                    activate(btAddDetectedToPopBets, btAddAllToPopBets, btAddAllToBetOfTheDay);
                    listenerForSelectionEvenIfLocked();
                } else {
                    inactivate(btAddDetectedToPopBets, btAddAllToPopBets, btAddAllToBetOfTheDay);
                }
                return;
            } else {
        ...
        function initBetOfTheDay() {
            if (getIsThereBetOfTheDay()) {
                activate(btClearBetOfTheDays);
            } else {
                inactivate(btClearBetOfTheDays);
                betOfTheDayCounter = 0;
            };
        }

        window.clearBetOfTheDays = () => {
            if (getIsThereBetOfTheDay()) {
                getState().sportsbook.betOfTheDay.bets = []
        ...
        function getIsThereBetOfTheDay() {
            return getState().sportsbook?.betOfTheDay?.bets?.length > 0 || false;
        }

        window.addAllSelectionsFromBetslipToBetOfTheDay = () => {
            ...
            getAreAllSelectionsPrematch(selectionIdArray) ?
                hide(notAllBetOfTheDaySelectionsPrematchMessage) :
                show(notAllBetOfTheDaySelectionsPrematchMessage);
        };

        function addSelectionToBetOfTheDay(selectionIds, marketIds, eventIds) {
            betOfTheDayCounter++;
            const longText = chkLongBetOfTheDayTitles.checked 
                ? " being a long text for testing purposes. The Expectation is ellipsis truncation, no multiline display" 
                : "";
            const analysisTitle = `SBTool Bet Analysis Title ${betOfTheDayCounter}${longText}`;
            const previewTitle = `SBTool Preview Title ${betOfTheDayCounter}${longText}`;

            const bet = {
                ambassadorImage: {
                    url: "https://betssongroup.github.io/sportsbook/qa/sportsbook-tool/backgounds/sbtool_betoftheday_transparent.png",
                    altText: "",
                    width: 718,
                    height: 424
                },
            ...
            getState().sportsbook.betOfTheDay.bets.push(bet);
            triggerChangeDetection();
            initBetOfTheDay();
        }
