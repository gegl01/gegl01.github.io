(function () {
    ...
    function initSbToolsSelection(scope) {
        ...
        const btRemoveAllFromPopBets = document.getElementById("btRemoveAllFromPopBets");
        const popularBetsControls = document.getElementById("popularBetsControls");
        const popularBetsMessage = document.getElementById("popularBetsMessage");
        ...
        function listenerForSelection() {
            ...
            if (isHomePage != previousIsHomePage) {
                ...
                if (isHomePage) {
                    ...
                    hide(popularBetsMessage);
                } else {
                    hide(popularBetsControls);
                    show(popularBetsMessage);
                }
            }
            ...
            if (isPopularBetsMoreThanZero != previousIsPopularBetsMoreThanZero) {
                ...
                if (isPopularBetsMoreThanZero) {
                    activate(btRemoveAllFromPopBets);
                } else {
                    inactivate(btRemoveAllFromPopBets);
                }
            }
        ...
        function addSelectionToPopularBets(selectionId) {
            ...
            if (isMarketPreBuilt(marketId)) {
                ...
                log("addSelectionToPopularBets called with " + selectionId);
            } else {
        ...
        //     popularPreBuiltBets.events = [eventObject];
        //     obgState.sportsbook.popularPreBuiltBets = popularPreBuiltBets;
        // }

        function addSelectionToPopularPreBuiltBets(eventId, selectionId) {
            log("addSelectionToPopularPreBuiltBets called");
            let popularPreBuiltBets;
            if (!obgState.sportsbook.popularPreBuiltBets.events) {
                popularPreBuiltBets = getEmptyPopularPreBuiltBets();
            } else {
            ...
            // Check if eventObject exists for the given eventId
            let isEventAlreadyInPopularPreBuiltBets = false;
            for (let ev of popularPreBuiltBets.events) {
                if (ev.id = eventId) {
                    isEventAlreadyInPopularPreBuiltBets = true;
                    break;
                }
            }
            log("isEventAdded: " + isEventAlreadyInPopularPreBuiltBets);

            if (isEventAlreadyInPopularPreBuiltBets) {
                log("eventObject: " + eventObject.id)
                // Check if selectionIds array exists, if not, create it
                if (!eventObject.selectionIds) {
                    log("selectionIds array created ");
                    eventObject.selectionIds = [];
                }
                // Check if selectionId already exists in the eventObject.selectionIds array
                if (!eventObject.selectionIds.includes(selectionId)) {
                    log("selectionId " + selectionId + " not yet in the eventObj");
                    // Add selectionId to the eventObject.selectionIds array
                    eventObject.selectionIds.push(selectionId);
                }
            } else {
                // If eventObject does not exist, create a new eventObject
                // eventObject = {
                //     eventId: eventId,
                //     selectionIds: [selectionId]
                // };
                // Add the new eventObject to popularPreBuiltBets.events array
                log("eventObject pushed: " + eventObject.id)
                eventObject.selectionIds = [selectionId];
                popularPreBuiltBets.events.push(eventObject);
            }

            // Update the popularPreBuiltBets in the state
            obgState.sportsbook.popularPreBuiltBets = popularPreBuiltBets;
        }
    ...
    function isMarketPreBuilt(marketId) {
        ...
        return (preBuiltMarketTemplateIds.includes(getMarketTemplateId(marketId)));
    }
    ...
    function getIsAndreasChangeDeployed() {
