(function main() {
    ...
    const IS_XSISMFEOVERRIDEAPPLIED_EXPOSED = isDefined("xSbIsMfeOverrideApplied");
    const IS_SBXPSPORTSBOOKAPPVERSION_EXPOSED = isDefined("sbXpSportsbookAppVersion");
    ...
    function checkEnabledFeatures() {
        ...
        if (sportsbookToolFeatures === null) {
            removeFeature("setSegmentSection");
        }
    }
    ...
    function getIsB2BIframeOnly() {
        if (IS_SBMFESSTARTUPCONTEXT_EXPOSED) return false;
    ...
    //     log("getHostPageEnvironment called");
    //     if (IS_PAGECONTEXTDATA_EXPOSED && !IS_SBXPSPORTSBOOKAPPVERSION_EXPOSED) {
    ...
    //         return env;
    //     }
    //     if (IS_OBGSTARTUP_EXPOSED) return obgStartup.config.appSettings.environment;
    //     return getEnvByURL(location.href);
    // }
    ...
    function initContext() {
        ...
        const mfeEnvMismatchSection = getElementById("mfeEnvMismatchSection");
        ...
        const btOpenMatchingMfeEnv = getElementById("btOpenMatchingMfeEnv");
        const notMatchingMfeEnvSpan = getElementById("notMatchingMfeEnvSpan");
        ...
        if (IS_SPORTSBOOK_IN_IFRAME) {
            ...
            checkIframeForEnvMismatch();
        }

        else if (IS_SBMFESSTARTUPCONTEXT_EXPOSED) {
            hide(openIframeSection);
            IS_OBGSTATE_OR_XSBSTATE_EXPOSED && IS_OBGRT_EXPOSED ?
                hide(obgStateAndRtSection) :
                show(obgStateAndRtSection);
            checkForMfeEnvMismatch();
        }
        ...
        // function checkIframeForEnvMismatch() {
        ...
        function handleIfIframeEnvNotMatching() {
            ...
            if (SB_ENVIRONMENT != HOST_PAGE_ENVIRONMENT) {
                ...
                btOpenMatchingIframe.innerText = HOST_PAGE_ENVIRONMENT.toUpperCase() + " iframe";
                btOpenFullPageWithMatchingIframe.innerText = HOST_PAGE_ENVIRONMENT.toUpperCase() + " full";
        ...
        function checkForMfeEnvMismatch() {
            if (SB_ENVIRONMENT != HOST_PAGE_ENVIRONMENT) {
                show(mfeEnvMismatchSection);
                hide(obgStateAndRtSection);
                btOpenMatchingMfeEnv.innerText = HOST_PAGE_ENVIRONMENT.toUpperCase();
                notMatchingMfeEnvSpan.innerText = " " + SB_ENVIRONMENT.toUpperCase() + " ";
            } else {
    ...
    function isSbVersionAtLeast(majorReq, minorReq) {
        const [major, minor] = SB_VERSION
            .split("-")[0]
            .split(".")
            .map(Number);
    ...
    function reloadPageWithFeature(feature) {
        ...
        switch (feature) {
            ...
            case "openMatchingIframe":
                url = new URL(replaceEnvInIframeURL(getCleanIframeURL(iframeURL)) + getPathAndParamsFromSportsbookURL(getCurrentPageUrl()));
                params.push(EXPOSE_OBGSTATE, EXPOSE_OBGRT);
                disableSealStoreIfNeeded();
                messageRowId = "openMatchingIframeRow";
                break;
            // case "overrideSbMfeEnv":
            //     HOST_PAGE_ENVIRONMENT == "alpha" ?
            //         params.push(new URLParam("overrideSbMfeEnvironment", "alpha")) : params.push(new URLParam("overrideSbMfeEnvironment", "test"));
            //     if (!IS_OBGSTATE_OR_XSBSTATE_EXPOSED) params.push(EXPOSE_OBGSTATE, EXPOSE_OBGRT);
            //     messageRowId = "overrideMfeSbEnvRow";
            //     break;
            case "openIframe":
                url = new URL(getCleanIframeURL(iframeURL) + getPathAndParamsFromSportsbookURL(getCurrentPageUrl()));
                params.push(EXPOSE_OBGSTATE, EXPOSE_OBGRT);
                disableSealStoreIfNeeded();
    ...
    function getDeepLink() {
        var deepLink = new URL(getCurrentPageUrl());
    ...
    window.openIframeFromMfe = () => {
        const playground = BRANDS[BRAND_NAME].playground;
        ...
        const staticContextId = (IS_OBGSTATE_OR_XSBSTATE_EXPOSED ?
            getState()?.b2b?.userContext?.staticContextId
            : undefined) ?? sbMfeStartupContext?.contextId?.staticContextId;

        const userContextId = (IS_OBGSTATE_OR_XSBSTATE_EXPOSED ?
            getState()?.b2b?.userContext?.userContextId
            : undefined)
            ?? sbMfeStartupContext?.contextId?.userContextId;

        window.open(`${baseUri}/${staticContextId}/${userContextId}${substringFromFifthSlash(getCurrentPageUrl())}`);
    ...
    function initSbToolsMarket(scope) {
        ...
        const chkIsEarlyPayoutAvailable = getElementById("chkIsEarlyPayoutAvailable");
        const chkIsSuperSubAvailable = getElementById("chkIsSuperSubAvailable");
        ...
        function initPlayerPropsAddition() {
            ...
            if (!sampleMarketCopy.marketTemplateTags.some(item => [101].includes(item))) { // { "bc_football_player-stats", 101 },
            ...
            window.addAnotherPlayerPropsMarket = () => {
                ...
                const isHomeTeam = getState().sportsbook.selection?.selections[sampleSelectionId]?.isHomeTeam;
        ...
        function initMarketPropertyCheckboxes() {
            ...
            chkIsEarlyPayoutAvailable.checked = getMarketTemplateTags(marketId).includes(163);
            chkIsSuperSubAvailable.checked = getMarketTemplateTags(marketId).includes(177);
        ...
        window.toggleMarketProperty = (property) => {
            switch (property) {
                ...
                case "betDistributionAvailable":
                    ...
                    trigger();
                    break;
                case "earlyPayoutAvailable":
                    toggleEarlyPayoutAvailable();
                    break;
                case "superSubAvailable":
                    toggleIsSuperSubAvailable();
                    break;
            }
            function trigger() {
                triggerMarketChangeDetection(marketId);
        ...
        function toggleEarlyPayoutAvailable() {
            toggleMarketPropertyByTag(163);
        }

        function toggleIsSuperSubAvailable() {
            toggleMarketPropertyByTag(177);
        }

        function toggleMarketPropertyByTag(tag) {
            let marketTags = [...marketTemplateTags];
            marketTags.includes(tag) ? updateMarketTemplateTags("remove", tag) : updateMarketTemplateTags("add", tag);
        }
        ...
        window.updateMarketTemplateTags = (addOrRemove) => {
            const tag = Number(fdAddMarketTemplateTag.value);
            updateMarketTemplateTags(addOrRemove, tag)
        };

        function updateMarketTemplateTags(addOrRemove, tag) {
            let marketTags = [...marketTemplateTags];
            if (tag == 0) return;
    ...
    function initSbToolsSelection(scope) {
        ...
        const betsOfTheDayNotEnabledMessage = getElementById("betsOfTheDayNotEnabledMessage");
        ...
        const isBetOfTheDayEnabledByConfig = getConfig().sportsbook?.betOfTheDay?.isEnabled;

        if (isBetOfTheDayEnabledByConfig) {
            show(betsOfTheDayControls);
            hide(betsOfTheDayNotEnabledMessage);
        } else {
            hide(betsOfTheDayControls);
            show(betsOfTheDayNotEnabledMessage);
        }
        ...
        function listenerForSelection() {
            ...
            if (isPageEligible != previousIsPageEligible) {
                ...
                if (isPageEligible) {
                    ...
                    isBetOfTheDayEnabledByConfig ? show(betsOfTheDayControls) : hide(betsOfTheDayControls);
                    hide(popularBetsNotEligiblePageMessage, betsOfTheDayNotHomeMessage);
                } else {
                    hide(popularBetsControls, betsOfTheDayControls);
                    show(popularBetsNotEligiblePageMessage);
                    isBetOfTheDayEnabledByConfig ? show(betsOfTheDayNotHomeMessage) : hide(betsOfTheDayNotHomeMessage);
                }
            }
    ...
    function initSbToolsEvent(scope) {
        ...
        const jerseyNumberSection = getElementById("jerseyNumberSection");
        ...
        const playerSpecialsPlayerCountRange = getElementById("playerSpecialsPlayerCountRange");
        const playerSpecialsPlayerCount = getElementById("playerSpecialsPlayerCount");

        const upsideJackpotSelectionRange = getElementById("upsideJackpotSelectionRange");
        const upsideJackpotSelectionCount = getElementById("upsideJackpotSelectionCount");
        ...
        const createPlayerSpecialsSection = getElementById("createPlayerSpecialsSection");
        const playerSpecialsCategories = getElementById("playerSpecialsCategories");

        const createUpsideJackpotSection = getElementById("createUpsideJackpotSection");
        ...
        window.setStartingTime = () => {
            const amount = Number(getElementById('timeAmount').value);
            const unit = getElementById('timeUnit').value;
        ...
        function showCreationSectionForMarketType(marketType) {
            ...
            switch (marketType) {
                ...
                case "fastMarket":
                    ...
                    break;
                case "playerSpecials":
                    initCreatePlayerSpecialsMarketSection();
                    break;
                case "upsideJackpot":
                    initCreateUpsideJackpotMarketSection();
                    break;
            }
        }
        ...
        window.createMarket = () => {
            ...
            switch (marketType) {
                case "playerProps":
                    ...
                    break;
                case "playerSpecials":
                    tagsArray = MARKET_TEMPLATE_TAGS_FOR_PLAYER_SPECIALS;
                    createPlayerSpecialsMarket();
                    break;
                case "preBuilt":
                ...
                case "fastMarket":
                    ...
                    break;
                case "upsideJackpot":
                    tagsArray = MARKET_TEMPLATE_TAGS_FOR_UPSIDE_JACKPOT;
                    createUpsideJackpotMarket();
                    break;
            }
            ...
            function createPlayerPropsMarket() {
                ...
                const displayTeamFilterChips = getElementById("chkDisplayTeamFilterChipsPProps").checked;
                const homeTeamHighlighted = getElementById("chkHomeTeamHighlightedPProps").checked;
            ...
            function createPlayerSpecialsMarket() {
                const marketKey = getElementById("playerSpecialsSelector").value;

                const homeTeamHighlighted = getElementById("chkHomeTeamHighlightedPSpecials").checked;
                const displayTeamFilterChips = getElementById("chkDisplayTeamFilterChipsPSpecials").checked;
                const marketTemplateTags = [...MARKET_TEMPLATE_TAGS_FOR_PLAYER_SPECIALS, 14, 164];
                if (displayTeamFilterChips) marketTemplateTags.push(165);

                let players = getRandomElementsFromArray(subParticipants, playerSpecialsPlayerCountRange.value);

                const templateMap = {
                    playerFirstAttemptOnGoal: { mti: "PLYPROPNXTSHTLOC", label: "Player First Attempt On Goal" },
                    playerShotLocation: { mti: "PLYPROPNXTSHTLOC", label: "Player Shot Location" }
                };

                const marketTemplateId = templateMap[marketKey].mti;
                const playerSpecialsMarketName = `${templateMap[marketKey].label} - (NEW) by SB Tool`;
                ...
                for (let player of players) {
                    const isHomeTeam = player.side == 1;
                    createMarketForPlayer(player, isHomeTeam);
                }

                function createMarketForPlayer(player, isHomeTeam) {
                    const noWhiteSpacePlayer = replaceWhitespaceWithDash(player.label)
                    const marketTemplateTags2 = [...marketTemplateTags];
                    ...
                    if (marketKey == "playerShotLocation") {
                        createSelection(isHomeTeam, player, "Shot Inside Box", "SHOTINSIDEBOX", 1);
                        createSelection(isHomeTeam, player, "Shot Outside Box", "SHOTOUTSIDEBOX", 2);
                    }
                }
                ...
                function createSelection(isHomeTeam, player, label, selectionTemplateId, sortOrder) {
                    ...
                    selection.selectionTemplateId = selectionTemplateId;
                }
        ...
        function createUpsideJackpotMarket() {
            const marketTemplateId = "UPSDJKT";
            const marketIdRandomNumber = getRandomInt(1000, 9999);
            const marketId = "m-" + eventId + "-" + marketTemplateId + "-" + marketIdRandomNumber;
            // const marketLabel = "Upside Jackpot by SB Tool - " + marketIdRandomNumber;
            const marketLabel = getState()?.translation?.translations["event-page.upside.goal-fiesta.market-title"] || "Goal Rush";
            ...
            obgRt.createMarket(
                ...
                marketId,
                marketTemplateId,
                MARKET_TEMPLATE_TAGS_FOR_UPSIDE_JACKPOT,
                marketLabel,
                0, //lineValue
                1, //columnLayout
                50, //sortOrder
            ...
            const selectionCount = Number(upsideJackpotSelectionRange.value) + 1;
            let selectionTemplateId;
            let selectionCounter = 0;

            for (let i = 0; i <= selectionCount - 1; i++) {
                selectionCounter++;
                if (i < 1) {
                    selectionTemplateId = "PHS";
                } else {
            ...
            function createSelection(selectionTemplateId, selectionCounter) {
                ...
                const selectionId = "s-" + marketId + "-" + selectionTemplateId.toLowerCase();
                const goalCount = selectionCounter - 2;
                const label = goalCount === -1
                    ? "Fabricated Upside Selection"
                    : goalCount < 2
                        ? `${goalCount} ${goal} `
                        : `${goalCount} ${goals}`;
                ...
                if (selectionCounter == 1) {
                    selectionSpecifics = {
                        values: {
                            line: {
                                specifierType: "Line",
                                values: {
                                    order: selectionCounter.toString()
                                }
                            }
                        }
                    };
                    ...
                    selectionSpecifics = {
                        values: {
                            ...
                            upside: {
                                specifierType: "Upside",
                                values: {
                                    linesOfInterest: "Goals",
        ...
        function initCreatePlayerPropsMarketSection() {
            ...
            hide(createFastMarketSection, createPreBuiltMarketSection, createPlayerSpecialsSection, createUpsideJackpotSection);
        ...
        function initCreateUpsideJackpotMarketSection() {
            show(createUpsideJackpotSection);
            hide(createFastMarketSection, createPreBuiltMarketSection, createPlayerSpecialsSection, createPlayerPropsSection);
            activate(btCreateMarket);

            upsideJackpotSelectionCount.textContent = upsideJackpotSelectionRange.value;
            upsideJackpotSelectionRange.addEventListener('input', () => {
                upsideJackpotSelectionCount.textContent = upsideJackpotSelectionRange.value;
            });
        ...
        function initCreatePlayerSpecialsMarketSection() {
            show(createPlayerSpecialsSection);
            hide(createFastMarketSection, createPreBuiltMarketSection, createPlayerPropsSection, createUpsideJackpotSection);

            const allowedCategoryIds = getCategoriesWithMarketTabOnEventPage(MARKET_TEMPLATE_TAGS_FOR_PLAYER_SPECIALS);

            playerSpecialsCategories.innerText = getCategoryNamesByCategoryIds(allowedCategoryIds);

            if (allowedCategoryIds.includes(categoryId)) {
                activate(btCreateMarket, createPlayerSpecialsSection);
                createMarketMessage.innerText = null;
            } else {
                displayInRed(createMarketMessage);
                createMarketMessage.innerText = "Not for this category";
                inactivate(btCreateMarket, createPlayerSpecialsSection);
            }

            initSubparticipants(eventId);

            playerSpecialsPlayerCountRange.setAttribute("max", subParticipants.length);

            playerSpecialsPlayerCount.textContent = playerSpecialsPlayerCountRange.value;

            playerSpecialsPlayerCountRange.addEventListener('input', () => {
                playerSpecialsPlayerCount.textContent = playerSpecialsPlayerCountRange.value;
            });
        }
        ...
        function initCreateFastMarketSection() {
            ...
            hide(createPlayerPropsSection, createPreBuiltMarketSection, createPlayerSpecialsSection, createUpsideJackpotSection);
        ...
        function initCreatePreBuiltMarketSection() {
            ...
            hide(createFastMarketSection, createPlayerPropsSection, createPlayerSpecialsSection, createUpsideJackpotSection);
        }
        ...
        const chkJerseyNumbers = getElementById("chkJerseyNumbers");


        window.setParticipantLogo = (logoType) => {
            // const wantJerseyNumbers = chkJerseyNumbers.checked;
            ...
            //     jersey: {
            //         type: 2,
            //         urlFn: () => getRandomFootballJerseyUrl(wantJerseyNumbers),
            //         colours: () => [getRandomColor(), getRandomColor()]
            //     },
            // };
    ...
    //         }
    //         return getCurrentPageUrl().includes("/ctx");
    //     }
    ...
    function getIsUserLoggedIn() {
        ...
        if (IS_B2B_IFRAME_ONLY) {
            ...
            if (getCurrentPageUrl().includes("/ctx")) return true;
        }
    ...
    window.initBonuses = () => {
        ...
        function getRelativeLinkForGoToEventPage() {
            ...
            if (IS_B2B_IFRAME_ONLY) {
                ...
                urlString = origin + "/"
                    + sbMfeStartupContext?.userContext?.contextInformation?.interfaceSettings?.brandSportsbookPath + "/"
                    + slug;
            } else {
                urlString = origin + "/"
                    + LANGUAGECODE + "/"
            ...
            function normalizeUrl(urlString) {
                return urlString.replace(/(^https?:\/\/)|\/{2,}/g, (match, protocol) => {
        ...
        function selectPb(key) {
            ...
            if (radioPbGarbage.checked) {
                ...
                if (getIsCombinationPriceBoost(selectedPriceBoost.id)) {
                    ...
                    hide(boostedSelectionDiv, goToEventPageRow);
                } else {
                    ...
                    show(boostedSelectionDiv, goToEventPageRow);
                }
            }
    ...
    function getJurisdiction() {
        return (
            (IS_OBGCLIENTENVIRONMENTCONFIG_STARTUPCONTEXT_EXPOSED
                ? obgClientEnvironmentConfig.startupContext?.userContext?.contextInformation?.jurisdiction
                : undefined) ??
            (IS_SBMFESSTARTUPCONTEXT_EXPOSED
                ? sbMfeStartupContext?.userContext?.contextInformation?.jurisdiction
                : undefined) ??
            (IS_OBGSTATE_OR_XSBSTATE_EXPOSED && IS_B2B_IFRAME_ONLY
                ? getState().b2b?.userContext?.contextInformation?.jurisdiction
                : undefined) ??
            (IS_OBGSTATE_OR_XSBSTATE_EXPOSED
                ? getState()?.jurisdiction?.jurisdiction
                : undefined)
        );
    }

    function getFacadeId() {
        return (
            (IS_SBMFESSTARTUPCONTEXT_EXPOSED
                ? sbMfeStartupContext?.userContext?.contextInformation?.facadeId
                : undefined) ??
            (IS_OBGCLIENTENVIRONMENTCONFIG_STARTUPCONTEXT_EXPOSED
                ? obgClientEnvironmentConfig.startupContext?.userContext?.contextInformation?.facadeId
                : undefined) ??
            (IS_OBGSTATE_OR_XSBSTATE_EXPOSED && IS_B2B_IFRAME_ONLY
                ? getState().xSbState.b2b?.userContext?.contextInformation?.facadeId
                : undefined)
        );
    }
    ...
    function getBrandId() {
        return (
            (IS_SBMFESSTARTUPCONTEXT_EXPOSED
                ? sbMfeStartupContext?.brandId
                : undefined) ??
            (IS_OBGCLIENTENVIRONMENTCONFIG_STARTUPCONTEXT_EXPOSED
                ? obgClientEnvironmentConfig?.startupContext?.brandId
                : undefined)
        );
    }
    ...
    function initSegments() {
        ...
        const facadeIdSpan = getElementById("facadeIdSpan");
        const brandIdSpan = getElementById("brandIdSpan");
        ...
        function listenerForSegments() {
            ...
            if (previousSegmentGuid == segmentGuid) {
                ...
                jurisdisctionSpan.innerText = (getJurisdiction() ?? "undefined").toUpperCase();
                segmentGuidSpan.innerText = segmentGuid;
                facadeIdSpan.innerText = (getFacadeId() ?? "undefined").toLowerCase();
                brandIdSpan.innerText = (getBrandId() ?? "undefined").toLowerCase();
                fillSegmentName();
                fillSegmentLegacyId();
    ...
    function getRandomTeamLogoUrl() {
        const teams = [
            "07f3ec84584f4b1795c0002c10227ad3",
    ...
    window.overrideSportsbookMfe = async () => {
        const shouldRestart = await overrideSportsbookMfe(HOST_PAGE_ENVIRONMENT);
        if (shouldRestart) {
            return main(); // restart parent
        }
    }

    async function overrideSportsbookMfe(environment) {
        const OVERRIDE_FLAG = "xSbIsMfeOverrideApplied";

        const VALID_ENVIRONMENTS = ["test", "qa", "alpha", "prod"];

        const MFE_TAG_NAME = "SB-XP-SPORTSBOOK";

        const ASSET_URL_PATTERN =
            /\/dist\/(?:test|qa|alpha|prod)\/xp\/widgets\/sportsbook\/([0-9a-zA-Z-]+)\/[0-9a-zA-Z.-]+\/(desktop|mobile)\/files\/.+\.(?:js|css)$/;

        const HOSTS = {
            BDE: "sbplayground1.net",
            BLE: "sbplayground.net",
        };

        const logger = {
            log: (message, ...args) =>
                console.log(`[mfe-override] ${message}`, ...args),

            warn: (message, ...args) =>
                console.warn(`[mfe-override] ${message}`, ...args),

            error: (message, ...args) =>
                console.error(`[mfe-override] ${message}`, ...args),
        };
        ...
        if (!environment) {
            logger.warn("Environment is required.");
            return;
        }
        ...
        if (!VALID_ENVIRONMENTS.includes(environment)) {
            logger.warn(
                `Invalid environment "${environment}". Valid values: ${VALID_ENVIRONMENTS.join(", ")}`
            );
            return;
        }

        if (window[OVERRIDE_FLAG]) {
            logger.warn(
                "MFE override has already been applied. Aborting duplicate override."
            );
            return;
        }
        ...
        function deepQuerySelector(root, selector) {
            ...
            for (const shadowRoot of shadowRoots) {
                const result = deepQuerySelector(shadowRoot, selector);
        ...
        function deepQuerySelectorAll(root, selector) {
            ...
            for (const shadowRoot of shadowRoots) {
                matches.push(
                    ...deepQuerySelectorAll(shadowRoot, selector)
                );
            }
        ...
        function getEnvironmentPrefix(env) {
            return env !== "prod" ? `${env}.` : "";
        }

        function getHostType(env) {
            switch (env) {
                case "test":
                case "qa":
                    return "BDE";
                ...
                default:
                    throw new Error(
                        `Unknown environment type for "${env}"`
                    );
            }
        }

        function getTargetBaseUrl(env) {
            const prefix = getEnvironmentPrefix(env);
            const host = HOSTS[getHostType(env)];

            return `https://d-cf.${prefix}${host}`;
        }

        function getIndexerUrl(baseUrl, env) {
            return `${baseUrl}/dist/${env}/xp/widgets/sportsbook/indexer.json`;
        }

        async function fetchIndexer(baseUrl, env) {
            const url = getIndexerUrl(baseUrl, env);
            ...
            try {
                ...
                if (!response.ok) {
                    logger.warn(
                        `Failed to fetch indexer from ${url}: ${response.status} ${response.statusText}`
                    );
                ...
                logger.warn(
                    `Error fetching indexer from ${url}:`,
                    error
                );
            }
        }

        function getAssetInfo(assetUrl) {
            const match = assetUrl.match(ASSET_URL_PATTERN);

            if (!match) {
                logger.warn(
                    `URL does not match expected pattern.\nURL: ${assetUrl}\nPattern: ${ASSET_URL_PATTERN}`
                );
            ...
            const deviceType = match[2];

            if (!brandId || !deviceType) {
                logger.warn(
                    `Could not extract brandId or deviceType from URL: ${assetUrl}`
                );
        ...
        class SportsbookOverrideManager {
            constructor(environment) {
                ...
                this.targetBaseUrl = getTargetBaseUrl(environment);

                const assetElements = deepQuerySelectorAll(
                    document,
                    `
                script[type='module'],
                ...
                for (const element of assetElements) {
                    ...
                    if (!ASSET_URL_PATTERN.test(assetUrl)) {
                        continue;
                    }
                    ...
                    if (
                        ...
                        this.existingAssets.css.push(element);
                    } else {
                        this.existingAssets.js.push(element);
                    }

                    this.assetInfo ??= getAssetInfo(assetUrl);
                }

                if (
                    !this.existingAssets.js.length &&
                    !this.existingAssets.css.length
                ) {
                    throw new Error(
                        "No existing MFE assets found."
                    );
                }

                logger.log(
                    `Detected ${this.existingAssets.js.length} JS assets and ${this.existingAssets.css.length} CSS assets for override.`
                );

                const mfeElement = deepQuerySelector(
                    document,
                    MFE_TAG_NAME
                );
                ...
                if (!mfeElement) {
                    throw new Error(
                        `MFE element with tag ${MFE_TAG_NAME} not found in document.`
                    );
                }

                this.mfeElement = mfeElement;
            }

            updateOverrideAssets(jsBundles, cssBundles) {
                for (const bundleUrl of jsBundles) {
                    const script = document.createElement("script");

                    script.type = "module";

                    script.src = isRelativeUrl(bundleUrl)
                        ? this.targetBaseUrl + bundleUrl
                        : bundleUrl;

                    this.overrideAssets.js.push(script);
                }

                for (const bundleUrl of cssBundles) {
                    const link = document.createElement("link");

                    link.rel = "stylesheet";

                    link.href = isRelativeUrl(bundleUrl)
                        ? this.targetBaseUrl + bundleUrl
                        : bundleUrl;

                    this.overrideAssets.css.push(link);
                }
            }

            async applyOverride() {
                // Remove existing JS assets
                for (const script of this.existingAssets.js) {
                    script.remove();
                }
                ...
                this.mfeElement.replaceWith(placeholder);

                try {
                    // Load new JS assets
                    for (const script of this.overrideAssets.js) {
                ...
                // Remove existing CSS
                for (const stylesheet of this.existingAssets.css) {
                    stylesheet.remove();
                }

                // Add new CSS
                for (const stylesheet of this.overrideAssets.css) {
                    document.head.appendChild(stylesheet);
                }

                // Recreate MFE element
                placeholder.replaceWith(
                    cloneElement(this.mfeElement)
                );
            }
        ...
        logger.log(
            `Override detected. Target environment: ${environment}`
        );

        let manager;

        try {
            manager = new SportsbookOverrideManager(environment);
        } catch (error) {
            logger.warn(error);
            return;
        }

        const assetInfo = manager.assetInfo;

        if (!assetInfo) {
            logger.warn(
                "Could not determine brandId or deviceType from existing assets."
            );

            return;
        }

        const { brandId, deviceType } = assetInfo;

        logger.log(
            `Using brandId: ${brandId}, deviceType: ${deviceType}`
        );

        const indexer = await fetchIndexer(
            manager.targetBaseUrl,
            environment
        );

        if (!indexer) {
            logger.warn(
                "Failed to fetch indexer bundles. Aborting override."
            );

            return;
        }

        const jsBundles = indexer?.[brandId]?.[deviceType]?.js;
        const cssBundles = indexer?.[brandId]?.[deviceType]?.css;

        if (!jsBundles?.length && !cssBundles?.length) {
            logger.warn(
                `No bundles found for deviceType "${deviceType}" in brandId "${brandId}".`
            );

            return;
        }

        manager.updateOverrideAssets(jsBundles, cssBundles);

        await manager.applyOverride();

        window[OVERRIDE_FLAG] = true;

        logger.log("MFE override successfully applied.");
        return true;
    }
})();