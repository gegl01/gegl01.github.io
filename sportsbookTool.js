(function () {
    ...
    const SB_TOOL_VERSION = "v1.6.107";
    ...
    function getIsB2BIframeOnly() {
        return (IS_OBGCLIENTENVIRONMENTCONFIG_EXPOSED && !!obgClientEnvironmentConfig?.startupContext?.contextId)
            || !!getState?.()?.b2b;
    }
        ...
        window.createMarket = () => {
            ...
            function createPlayerPropsMarket() {
                ...
                const playerPropsMarketName = `Player Props by SB Tool ${
                    oneSelectionsPerMarket ? "1" : "2"} selections per Market`;
                ...
                function createMarketForPlayer(player, lineValue) {                    
                    ...
                    marketId = `m-${eventId}-${marketTemplateId}-${noWhiteSpacePlayer}-${lineValue}-${getRandomInt(1000)}`;
                    marketIds.push(marketId);
                    obgRt.createMarket(
                        eventId,
                        marketId,
