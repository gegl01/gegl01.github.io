(function () {
    ...
    const IS_B2B = getIsB2B();
    const BRAND_NAME = getBrandName();
    const CULTURE = getCulture();
    const LANGUAGECODE = getLanguageCode();
    const BRANDWITHLANGUAGECODE = BRAND_NAME + " (" + LANGUAGECODE.toUpperCase() + ")";
    ...
    function getSbVersion() {
        ...
        if (IS_SPORTSBOOK_IN_IFRAME) {
            return "(Open the SB iframe to get this)";
        }
        try {
            appVersion = obgClientEnvironmentConfig.startupContext.appContext.version;
        } catch {
            try {
                appVersion = obgState.appContext.version;
            } catch {
                return "Data not available"
            }
        }
        if (IS_B2B) {
            return "SB-B2B-FE-" + appVersion;
        }
        return "OBGA-" + appVersion;
    }
    ...
    function getEnvironmentToDisplay() {
        ...
        if (getIsBleSource()) {
            return environmentToDisplay + ", using ALPHA API and data";
        }
    ...
    function getLanguageCode() {
        try {
            return obgState.market.currentMarket.languageCode;
        } catch {
            return obgClientEnvironmentConfig.startupContext.config.core.market.languageCode;
        }
    }

    function getIsEventInObgState(eventId) {
        return !!obgState.sportsbook.event.events[eventId];
    }

    function getIsSelectionInObgState(selectionId) {
        return !!obgState.sportsbook.selection.selections[selectionId];
    }

    function getIsMarketInObgState(marketId) {
        return !!obgState.sportsbook.eventMarket.markets[marketId];
    }
    ...
    function initContext() {
        ...
        document.getElementById("brandName").innerText = BRANDWITHLANGUAGECODE;
        ...
        function handleIfIframeEnvNotMatching() {
            ...
            if (ENVIRONMENT != iframeEnv) {
                ...
                btOpenNotMatchingIframe.innerText = iframeEnv.toUpperCase() + " iframe";
    ...
    function reloadPageWithFeature(feature) {
        ...
        switch (feature) {
            ...
            case "openMatchingIframe":
                url = new URL(replaceEnvInIframeURL() + window.location.search);
                params.push(ENABLE_OBGSTATE, ENABLE_OBGRT);
                messageRow = "openMatchingIframeRow";
                break;
            case "openNotMatchingIframe":
                url = new URL(iframeURL + window.location.search);
                params.push(ENABLE_OBGSTATE, ENABLE_OBGRT);
                messageRow = "openNotMatchingIframeRow";
                break;
            case "openIframe":
                url = new URL(iframeURL + window.location.search);
                params.push(ENABLE_OBGSTATE, ENABLE_OBGRT);
    ...
    function replaceEnvInIframeURL() {
        let iframeEnv = getIframeEnv();
        if (iframeEnv == "prod") {
    ...
    function getJiraTemplate() {
        var template =
            ...
            "|Brand(s)|" + BRANDWITHLANGUAGECODE + "|" + BRANDWITHLANGUAGECODE + "|\n" +
    ...
    function initSbToolsMarket(scope) {
        ...
        function listenerForMarket() {
            ...
            if (marketId === null) {
                ...
                //     sbIdForOddsManager.innerText = "Get Market ID for ISA/Redis & Odds Manager"
                // } else {
                //     sbIdForOddsManager.innerText = "Get Market ID for ISA/Redis"
                // }

                if (getEventPhase() == "Prematch") {
                    sbIdForOddsManager.innerText = "Get Market ID for ISA/Redis & Odds Manager"
                } else {
                    sbIdForOddsManager.innerText = "Get Market ID for ISA/Redis"
                }
        ...
        window.addMarketToCarouselOrCards = () => {
            ...
            createEmtyCarouselIfDoesntExist();
    ...
    function createEmtyCarouselIfDoesntExist() {
    ...
    function getIsPageValidForCarousel() {
        return isPageCardCapable() || isPageHomePage();
    }
    ...
    function getThreeColumnLayouts() {
        ...
        threeColumnLayouts = getThreeColumnLayoutsFromEventWidget();
        if (threeColumnLayouts == undefined) {
    ...
    function getThreeColumnLayoutsFromEventWidget() {
        try {
    ...
    function isBLE() {
        if (ENVIRONMENT_TO_DISPLAY == "PROD" || ENVIRONMENT_TO_DISPLAY == "ALPHA") {
            return true;
        } return false;
    }
    ...
    function initSbToolsEvent(scope) {
        ...
        function toggleHasPriceBoost() {
            ...
            function createPriceBoost(eventId, marketId, selectionId) {
                let sbtPbId = "SBTOOL-" + generateGuid();
                let priceBoostStateItem = {
                    "id": sbtPbId,
                    "name": "Generated by SB Tool, can't lookup in TT",
                    "type": "PriceBoost",
                    "bonusData": {
                        "type": "Mocked"
                    },
                ...
                obgState.sportsbook.priceBoost.priceBoost[sbtPbId] = priceBoostStateItem;
            }
        }
        ...
        function initEventPropertyCheckboxes() {
            ...
            if (!!obgState.sportsbook.scoreboard[eventId]) {
                obgState.sportsbook.scoreboard[eventId].varState == 2 ?
                    chkHasVar.checked = true :
                    chkHasVar.checked = false;
            } else {
                chkHasVar.checked = false;
            }
        }
    ...
    window.getLegacyEventId = () => {
        IS_BLE ?
            window.open("http://sbtradingmappingmainapi.ble.local/mapping/fixture/new/" + eventId) :
            window.open("http://sbtradingmappingmainapi.qa.bde.local/mapping/fixture/new/" + eventId);
    ...
    window.getLegacyMarketId = () => {
        IS_BLE ?
            window.open("http://sbtradingmappingmainapi.ble.local/mapping/market/new/" + marketId) :
            window.open("http://sbtradingmappingmainapi.qa.bde.local/mapping/market/new/" + marketId);
    ...
    window.initBonuses = () => {
        ...
        const radioPbNotApplicable = document.getElementById("radioPbNotApplicable");
        ...
        const freeBetEvetPhases = document.getElementById("freeBetEvetPhases");
        ...
        function listenerForPriceBoostDetails() {
            ...
            } else if (radioPbNotApplicable.checked) {
                listPriceBoostsBy("notApplicable");
            }
        }
        ...
        function populatePriceBoostSelectorBy(entityName) {
            pbSelector.innerHTML = "";
            var priceBoostsArray;

            if (entityName == "notApplicable") {
                priceBoostsArray = getArrayOfNotApplicablePriceboosts(Object.values(priceBoosts));
                priceBoostsArray = priceBoostsArray.sort((a, b) =>
                    a.name > b.name
                        ? 1 : -1);
            } else {
                priceBoostsArray = getArrayOfApplicablePriceboosts(Object.values(priceBoosts));
            ...
            pbNumberOf.innerText = Object.values(priceBoosts).length;
            pbNumberOfListed.innerText = priceBoostsArray.length;

            let option;
            for (var pb of priceBoostsArray) {
                option = document.createElement("option");
                if (entityName == "pbName" || entityName == "notApplicable") {
                    option.text = pb.name;
                } else if (entityName == "eventName") {
                    labelEventName = getEventDisplayLabel(pb.criteria.criteriaEntityDetails[0].eventId);
                    labelMarketTemplateId = "(" + getMarketTemplateId(pb.criteria.criteriaEntityDetails[0].marketId) + ")";
                    option.text = getEventDisplayLabel(pb.criteria.criteriaEntityDetails[0].eventId)
                        + " ("
                        + getMarketTemplateId(pb.criteria.criteriaEntityDetails[0].marketId)
                        + ")";
                }
                option.value = pb.id;
        ...
        function listPriceBoostsBy(value) {
            if (value == "eventName") {
                populatePriceBoostSelectorBy("eventName");
            } else if (value == "pbName") {
                priceBoosts = getPriceBoosts();
                populatePriceBoostSelectorBy("pbName");
            } else if (value == "notApplicable") {
                priceBoosts = getPriceBoosts();
                populatePriceBoostSelectorBy("notApplicable");
            }
        }

        function getArrayOfApplicablePriceboosts(priceBoostsArray) {
            let arr = [];
            for (let pb of priceBoostsArray) {
                if (getIsSelectionInObgState(pb.criteria.criteriaEntityDetails[0].marketSelectionId)) {
                    arr.push(pb);
                }
            }
            return arr;
        ...
        function getArrayOfNotApplicablePriceboosts(priceBoostsArray) {
            return priceBoostsArray.filter((element) => !getArrayOfApplicablePriceboosts(priceBoostsArray).includes(element));
        }
        ...
        function selectPb(key) {
            ...
            try {
                ...
                show(boostedSelectionDiv);
            } catch {
                displayInRed(pbPathToCompetition);
                hide(boostedSelectionDiv);
                pbPathToCompetition.innerHTML = "The boosted selection cannot be found. Probably not open anymore.";
                // boostedSelectionDiv.innerHTML = "---";
            }
